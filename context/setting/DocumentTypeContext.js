...
import { PRICEMATRIX_API, DOCUMENT_TYPE_API } from "utils/constant";
...
function DocumentTypeProvider({ children }) {
    ...
    async function GetItemList() {
        try {
            ...
            const response = await getApiData(DOCUMENT_TYPE_API.get, params, signal);
    ...
    async function DeleteItem(currentItem) {
        try {
            await apiCalling({ url: DOCUMENT_TYPE_API.delete + currentItem.document_type_id, method: 'delete', });
            setItemList((prevList) => prevList.filter((item) => item.document_type_id != currentItem.document_type_id))
            router.push(paths.setting.documentType);
        } catch (err) {
            console.log(err);
        }
    }
    ...
    return <DocumentTypeContext.Provider value={memoizedValue}>{children}</DocumentTypeContext.Provider>;
}
export { DocumentTypeContext, DocumentTypeProvider };
