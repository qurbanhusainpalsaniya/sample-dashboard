...
import { API_PAGE_LIMIT, FUEL_TYPE, SMALL_MODAL } from "utils/constant";
...
export default function FuelTypeMaster({ open, onClose, NeedtoSelect = false, selectedItem = [] }) {
    ...
    async function GetList() {
        try {
            ...
            const response = await getApiData(FUEL_TYPE.get, params, signal);
    ...
    async function AddItem(data) {
        try {
            const response = await apiCalling({ url: FUEL_TYPE.create, data, method: 'post', });
    ...
    async function UpdateItem(data) {
        try {
            const response = await apiCalling({ url: FUEL_TYPE.update + addModal.data?.fuel_type_id, data, method: 'put', })
            setResponseList(responseList.map(item => item.fuel_type_id === addModal.data?.fuel_type_id ? response.data.result : item));
            setUpdate(true);
        } catch (error) {
            console.error(error);
        }
    ...
    async function deleteItem(data) {
        try {
            await apiCalling({ url: FUEL_TYPE.delete + data?.fuel_type_id, method: 'delete', });
            setResponseList(responseList.filter(item => item.fuel_type_id != data?.fuel_type_id));
            setUpdate(true);
        } catch (error) {
            console.error(error);
        }
                ...
                <Box display={"flex"} flexDirection={"column"} minHeight={0} flex={1}>
                    <Box sx={{ p: 2, borderBottom: "1px solid", borderColor: "divider", }}>
                        <Box display={"flex"} flexDirection={"row"} justifyContent={"space-between"} alignItems={"center"} pb={1}>
                            <Typography variant={"h6"} textTransform='capitalize'>{t("fuel_type")} </Typography>
                            ...
                            <List sx={{ py: 0 }}>
                                {loading ? (
                                    ...
                                    responseList.map((row, index) => (
                                        <FuelTypeMasterRow
                                            key={index}
                                            selected={selectedItem?.includes(row.fuel_type_id)}
...
function FuelTypeMasterRow({ row, selected, onDelete, onView, onEdit, }) {
    const { title } = row;
    const { t } = useTranslate();
        ...
        <>
            <ListItem disablePadding>
                <ListItemButton selected={selected} onClick={onView}>
                    {selected && <GoogleMaterialIcon icon={'check'} sx={{ mr: 1, color: "primary.main" }} />}
                    <ListItemText
            ...
            {confirmation &&
                <ConfirmDialog
                    title={`${t("delete")}`}
                    description={t("msg_remove_from_order", { placeholder: title, placeholder2: t("fuel_type") })}
                    open={confirmation}
